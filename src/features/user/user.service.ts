...
export class UserService {
    ...
    async signupUser(user: SignUpDto): Promise<UserDocument> {
        try {
            // Check if user already exists
            const userRecord = await this.model.findOne({ mobile: user.mobile, deleted_dt: null });
            if (userRecord) throw new HttpException(ErrorConstants.USER_MOBILE_EXISTS, HttpStatus.BAD_REQUEST)
            const userEmailRecord = await this.model.findOne({ email: user.email, deleted_dt: null });
            if (userEmailRecord?.email == user.email) throw new HttpException(ErrorConstants.USER_EMAIL_EXISTS, HttpStatus.BAD_REQUEST)
            // Create new user document
            const userDocument: UserDocument = new this.model(user);
    ...
    async updateUserPassword(user: IUser): Promise<UserDocument> {
        try {
            let query: any = {
                'email': user.email,
                '$or': [
